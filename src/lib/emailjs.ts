...
const normalizeStatus = (status: string): 'CONFIRMED' | 'PENDING' | 'REJECTED' => {
...
const getStatusColor = (status_text: string) => {
  ...
  if (status_text === 'REJECTED') return '#D32F2F';
  return '#333333';
};
...
const generateMessageBody = (
  ...
  if (normalized === 'REJECTED')
    return `Unfortunately, your appointment request was not approved. Please contact us for more information or to reschedule.`;
